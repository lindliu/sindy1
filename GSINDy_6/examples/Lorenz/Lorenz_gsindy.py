...
#################### 3 variable ####################
x0_list = [[-8, 8, 27]]#, [-8, 8, 27], [-8, 8, 27], [-8, 8, 27]]
a_list = [(10,28,8/3)]#, (8,25,3), (9,30,2), (10,26,2)]
if num==1:
...
if __name__ == "__main__":
    ...
    # model_set, diff0_basis_list, diff1_basis_list, same0_basis_list, same1_basis_list, same2_basis_list, parameter_list = \
    #                             fit_gsindy_3d(sol_org_list, num_traj, num_feature, t_, num, real0, real1, real2, \
    #                                               monomial, monomial_name, precision, alpha, opt, deriv_spline, ensemble, print_results=False)
    
    # ms, best_BIC_model, parameter_list = model_selection_gsindy_3d( \
    #                                         x0, t_, a, real0, real1, real2, monomial, monomial_name, model_set, \
    ...
    for num in [1, len(a_list)]:
        ...
        if num==1:
            ...
            max_split = 5  ## split one series into several pieces
            for num_split in range(2, max_split+1):
                for idx in range(len(a_list)):
                    ...
                    model_set, diff0_basis_list, diff1_basis_list, same0_basis_list, same1_basis_list, same2_basis_list, parameter_list = \
                                                fit_gsindy_3d(sol_org_list, num_traj, num_feature, t_, num, real_list, \
                    ...
                    with open(save_path, "a") as file1:
                        ...
                        file1.write(f'basis of feature 2: {monomial_name[mask2]} \n\n')
        ...
        if num>1:
            ...
            for idx in range(num_traj):
                with open(save_path, "a") as file2:
                    ...
                    file2.write(f'coef of feature 2: {coef[idx,2,:][mask2[idx]]} \n')
                    file2.write(f'basis of feature 2: {monomial_name[mask2[idx]]} \n\n')
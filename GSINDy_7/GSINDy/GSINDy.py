...
class GSINDy():
    def __init__(self, monomial, monomial_name, \
        ...
        if len(monomial)==1:
            self.monomial0 = monomial[0]
            self.monomial0_name = monomial_name[0]
            self.monomial1 = monomial[0]
            self.monomial1_name = monomial_name[0]
            self.monomial2 = monomial[0]
            self.monomial2_name = monomial_name[0]
            
        if len(monomial)==2:
            self.monomial0 = monomial[0]
            self.monomial0_name = monomial_name[0]
            self.monomial1 = monomial[1]
            self.monomial1_name = monomial_name[1]
        ...
        if len(monomial)==3:
            ...
            self.monomial1 = monomial[1]
            self.monomial1_name = monomial_name[1]
            self.monomial2 = monomial[2]
            self.monomial2_name = monomial_name[2]
    ...
    def get_multi_sub_series_2D(self, sol_org_list, t, num_series=60, window_per=.7):
        ...
        for k in range(num_traj):
            for i in range(num_series):
                ...
                theta0_ = self.monomial0(np.c_[sol0_,sol1_])
                theta1_ = self.monomial1(np.c_[sol0_,sol1_])
    ...
    def get_multi_sub_series_3D(self, sol_org_list, t, num_series=60, window_per=.7):
        ...
        for k in range(num_traj):
            for i in range(num_series):
                ...
                theta0_ = self.monomial0(np.c_[sol0_,sol1_,sol2_])
                theta1_ = self.monomial1(np.c_[sol0_,sol1_,sol2_])
                theta2_ = self.monomial2(np.c_[sol0_,sol1_,sol2_])
                theta0.append(theta0_)
                theta1.append(theta1_)
    ...
    def basis_identification_(self, theta_, sol_deriv_, nth_feature):
        ...
        for epoch in range(self.max_iter):
            ...
            if self.plot_dist and nth_feature==0:
                plot_distribution(Xi0_group, nth_feature, epoch, self.monomial0_name, idx=self.idx_basis[idx_activ])
            if self.plot_dist and nth_feature==1:
                plot_distribution(Xi0_group, nth_feature, epoch, self.monomial1_name, idx=self.idx_basis[idx_activ])
            if self.plot_dist and nth_feature==2:
                plot_distribution(Xi0_group, nth_feature, epoch, self.monomial2_name, idx=self.idx_basis[idx_activ])
    ...
    def get_multi_theta(self, sol_org_list, t):
        ...
        for i in range(self.num_traj):
            ...
            for j in range(self.num_feature):
                if j==0:
                    theta_ = self.monomial0(sol_)
                if j==1:
                    theta_ = self.monomial1(sol_)
                if j==2:
                    theta_ = self.monomial2(sol_)

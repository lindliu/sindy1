...
if __name__ == "__main__":
    ...
    if exp_idx!=6:
        table_poly = np.zeros([num_traj+1, 9], dtype=object)
        table_poly[0,:] = ['$RMSE$', '$Mp$', '$Mr$', '$RMSE$', '$Mp$', '$Mr$', '$RMSE$', '$Mp$', '$Mr$']
        table_poly[1:,:3] = np.c_[rmse_gsindy_all_poly, mp_gsindy_all_poly, mr_gsindy_all_poly]
        table_poly[1:,3:6] = np.c_[rmse_gsindy_one_poly, mp_gsindy_one_poly, mr_gsindy_one_poly]
        table_poly[1:,6:9] = np.c_[rmse_sindy_poly, mp_sindy_poly, mr_sindy_poly]
    
        table_metrics = np.zeros([7,10], dtype=object)
        table_metrics[:,0] = ['Metric', 'traj. 1', 'traj. 2', 'traj. 3','traj. 4','traj. 5','traj. 6']
        table_metrics[:,1:] = table_poly

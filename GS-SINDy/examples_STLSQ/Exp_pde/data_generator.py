...
############################ Inviscid Burgers ########################
def ib_init(x, c):
    """Profile of the exact solution to the Inviscid Burgers for a single soliton on the real line."""   
...
u0 = ib_init(x-0.33*L, 0.75) + ib_init(x-0.65*L, 0.4)
# plt.plot(u0)

sol_ib = odeint(ib, u0, t, args=(L,), mxstep=5000)
plt.imshow(sol_ib.T[:,::4])

np.savez('data/IB_2.npz', t=t[::4], x=x, usol=sol_ib.T[:,::4])
#######################################################################
...
np.savez('data/KDV_2.npz', t=t[::4], x=x, usol=sol_kvd.T[:,::4])
#######################################################################

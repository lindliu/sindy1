...
for i in range(num_traj):
    ...
    sol0_deriv_org.append(sol_deriv_[:,[1]])
...
for k in range(num_traj):
    for i in range(num_series):
        sol0_, sol0_deriv_ = data_interp(np.linspace(length*(i*.005),length*(per+i*.005),num=int(length_sub//dt)), t, sol0_org[k].squeeze())
        sol1_, sol1_deriv_ = data_interp(np.linspace(length*(i*.005),length*(per+i*.005),num=int(length_sub//dt)), t, sol1_org[k].squeeze())
...
for _ in range(5):
    ...
    idx_similar = np.where(radius<1e-2)
